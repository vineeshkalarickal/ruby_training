...
puts hex.match?(reg_exp) ? "1 There is a match!" : "There is no match!"

# check to see if there is a match from the start of line
if hex =~ reg_exp_start
  puts "2 Yes, there is a match from the start of the line"
end

# check to see if the line ends with a hex color code
if hex =~ reg_exp_end
  puts "3 Yes, the line ends with a color hex code: #{hex.match(reg_exp_end)}"
end

if hex =~ reg_exp
  puts "4 Yes there is a match, starting at character #{hex =~ reg_exp}"
else
  puts "5 No match, sorry"
end
...
if hex.match?(reg_exp)
  puts "6 Here is the match: \" #{hex.match(reg_exp)}\""
  puts hex.match(reg_exp).class
  puts hex.match(reg_exp).length
  puts "9 These are the submatches: #{$1}, #{$2}"
end
...
puts "\n 10 Here are the matches using another method: "
m = hex.match(reg_exp)
for i in 0...m.length
	puts m[i]
end
